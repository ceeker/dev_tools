...
ARGS=`getopt -a -o N:C:R:S:I:X:J:U:T:P:M:K:c:h -l count:,client:,round:,sleeptime:,min:,max:,step:,runtime:,postfile:,Mult:,Keep:,Cookie:,help -- "$@"`
[ $? -ne 0 ] && usage
eval set -- "${ARGS}" 
...
if [ $flag != 0 ]; then
    ...
    cmd="$cmd -C $cookies -c$clients $url"
    runtest $cmd $outfile $rounds $sleeptime
fi
...
echo '本次测试结果如下：'
echo '+------+----------+----------+---------------+---------------+---------------+--------------------+--------------------+--------------------+--------------------+--------------------+'
echo '| 序号 |总请求数  |  并发数  | 失败请求数    |   每秒事务数  | 平均事务(ms)  | 并发平均事务数(ms) |     99%耗时(ms)    |     最大耗时(ms)   |     标准差         | 每秒传输数据(KB/s) |'
echo '+------+----------+----------+---------------+---------------+---------------+--------------------+--------------------+--------------------+--------------------+--------------------+'
...
ninenine=(`awk '/99%/{print $2F}' $outfile`)
max=(`awk '/100%/{print $2F}' $outfile`)
std=(`awk '/Total:/{print $4F}' $outfile`)
for ((i=0; i<${#comp[@]}; i++))
do
    echo -n "|"
    printf '%6s' $(($i+1)) 
